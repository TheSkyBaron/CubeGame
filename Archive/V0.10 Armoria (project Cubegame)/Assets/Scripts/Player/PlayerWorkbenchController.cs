...
namespace Game.Player
{
    public class PlayerWorkbenchController : MonoBehaviour
    {
        ...
        private InputAction Move, Look, Zoom, Hold, Place, RemoveSwitch, EditorModeSwitcher, Inventory, SlotSelector,BlockRotateZAxis,PauseMenu;
        private Rigidbody PlayerBody;
        private Camera PlayerCamera;
        private GameObject GhostObject, ActiveWorkbench, InventoryObject, PauseMenuPanel;
        private Material GhostMaterial;
        private Byte3 PlayerBlockRotationVector, LastRotation;
        private Vector2 PlayerCurrentRotation;
        private float PlayerCurrentZoom = 10;
        private bool Is_PlayerHoldingScreen, IsPlayerFrozen, Is_AlternativeLayout, Is_Removing;
        private Vector3 HitPointPlace, HitPointRemove, PlayerMovementVector;
        private int3 RotationCalculation = new();
        private Mesh GeneratedMesh;
        private BlockRenderData CalculatedData;

        private readonly int[] ToolbarSlots = new int[] { 1, 2, 1, 1, 1, 1, 1, 1, 1 };
        private int PlayerSelectedBlock = 1;
        private readonly float PlayerMinZoom = 5, PlayerMaxZoom = 100, FloatErrorPointAdjustment = 0.01f;
        ...
        private void Start()
        {
            ...
            PlayerBlockChange(PlayerSelectedBlock);
            GeneratedMesh = GameUtils.SingleBlockRenderer(CompiledBlockAtlas.BlockAtlas[PlayerSelectedBlock]);
        }
        ...
        private void OnEnable()
        {
            ...
            Inventory = InputMap.FindAction("Inventory");
            SlotSelector = InputMap.FindAction("SlotSelector");
            BlockRotateZAxis = InputMap.FindAction("BlockRotateZAxis");
            ...
            BlockRotateZAxis.performed += RotateBlockZAxis;
            PauseMenu.performed += GamePauseMenu;

            //Player Inventory Controlls
            Inventory.performed += ToggleInventory;
            SlotSelector.performed += SelectSlot;
        }
        ...
        private void PlayerPlaceBlock(InputAction.CallbackContext CTX)
        {
            if (!IsPlayerFrozen)
            {
                ...
                else if (HitPointPlace != -Vector3.one)
                {
                    ...
                    if (PlayerSelectedBlock != 0 || PlayerSelectedBlock != 1) Current.GridSystem.SetBlock(PlacePoint, PlayerSelectedBlock, PlayerBlockRotationVector);
                    else Current.GridSystem.SetBlock(PlacePoint, PlayerSelectedBlock, Byte3.Zero);
                }
        ...
        private void PlaceGhostObject(Vector3 Position, Vector3 Scale, Color GhostColor)
        {
            ...
            if(!LastRotation.Equals(PlayerBlockRotationVector))
            {
                CalculatedData = GameUtils.RotateBlock(CompiledBlockAtlas.BlockAtlas[PlayerSelectedBlock], new GridBlock() { ID = PlayerSelectedBlock, Rotation = PlayerBlockRotationVector });
                GeneratedMesh = GameUtils.SingleBlockRenderer(CalculatedData);
            }
            LastRotation = PlayerBlockRotationVector;
        ...
        public void PlayerBlockChange(int NewBlock)
        {
            ...
            {
                ID = NewBlock,
                Rotation = PlayerBlockRotationVector
            };
        ...
        public void SetToolbarSlot(int Slot, int ID)
        {
            ToolbarSlots[Slot] = ID;
        }

        private void SelectSlot(InputAction.CallbackContext CTX)
        {
            PlayerBlockChange(ToolbarSlots[(int)CTX.ReadValue<float>() - 1]);
        }

        private void RotateBlockZAxis(InputAction.CallbackContext CTX)
        {
            if(Is_AlternativeLayout)
            {
                float RotationAxis = CTX.ReadValue<float>();
                if (RotationAxis != 0)
                {
                    if (RotationAxis == 1) RotationCalculation.z += 3;
                    if (RotationAxis == -1) RotationCalculation.z += 1; // each +1 = clockwise turn from +Z
                    PlayerBlockRotationVector.z = (byte)(RotationCalculation.z % 4);
                }
            }
        }
        private void GamePauseMenu(InputAction.CallbackContext CTX)
        {
