    ...
    public class WorkbenchRenderer : IRenderer
    {
        public Mesh Calculate(GridBlock[] BuildableGrid, int3 Gridsize)
        {
            ...
            for (int i = 0; i < BuildableGrid.Length; i++)
            {
                ...
                BlockRenderData CalcuatedData = CalculateBlock(BuildableGrid[i]);
                int BlockVertexFaceLenght = CalcuatedData.BlockVerticies.GetLength(1);
                int BlockTrianglesFaceLenght = CalcuatedData.BlockTriangles.GetLength(1);
                ...
                for (int FaceIndex = 0; FaceIndex < CompiledBlockAtlas.BlockAtlas[BuildableGrid[i].ID].FaceCount; FaceIndex++)
                {
                    ...
                    int3 AdjacentBlock = CalculatedBlockPosition + CalcuatedData.FaceCheckVectors[FaceIndex];
                    int AdjacentBlockIndex = CommonMathematics.PositionToArray(AdjacentBlock, Gridsize);
                    if (!GameUtils.BlockFaceRenderCheck(AdjacentBlock,Gridsize,BuildableGrid,AdjacentBlockIndex,CalcuatedData,FaceIndex)) continue;

                    // Face looking at surface so we will render

                    for (int VertexIndex = 0; VertexIndex < BlockVertexFaceLenght; VertexIndex++)
                    {
                        if (CalcuatedData.BlockVerticies[FaceIndex, VertexIndex] == null) continue;
                        BlockVerticies[VertsCount + VertexIndex] = CalcuatedData.BlockVerticies[FaceIndex, VertexIndex] + new Vector3(CalculatedBlockPosition.x, CalculatedBlockPosition.y, CalculatedBlockPosition.z);
                    }

                    for (int TriangleIndex = 0; TriangleIndex < BlockTrianglesFaceLenght; TriangleIndex++)
                    {
                        if (CalcuatedData.BlockTriangles[FaceIndex, TriangleIndex] == -1) continue;
                        BlockTriangles[TrianglesCount + TriangleIndex] = CalcuatedData.BlockTriangles[FaceIndex, TriangleIndex] + VertexStartIndex;
                    }
        ...
        private BlockRenderData BlockRotate(Byte3 Rotation, BlockRenderData Original, Vector3 Scale)
        {
        ...
        {
            if (CurrentGridBlock.Rotation.Equals(Byte3.Zero)) return CompiledBlockAtlas.BlockAtlas[CurrentGridBlock.ID];
            BlockRenderData CalculatedBlock = GameUtils.DeepCopyBlockData(CompiledBlockAtlas.BlockAtlas[CurrentGridBlock.ID]);
