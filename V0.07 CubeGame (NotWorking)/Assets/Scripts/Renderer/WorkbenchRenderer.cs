    ...
    public class WorkbenchRenderer : IRenderer
    {
        public Mesh Calculate(GridBlock[] BuildableGrid, int3 Gridsize)
        {
            ...
            for (int i = 0; i < BuildableGrid.Length; i++)
            {
                ...
                BlockRenderData CalcuatedData = GameUtils.CalculateBlock(BuildableGrid[i]);
                ...
                int[] FaceCalculatedIndex = CalcuatedData.FaceCheckSquence;
                ...
                for (int FaceIndex = 0; FaceIndex < CompiledBlockAtlas.BlockAtlas[BuildableGrid[i].ID].FaceCount; FaceIndex++)
                {
                    ...
                    int3 AdjacentBlock = CalculatedBlockPosition + CalcuatedData.FaceCheckVectors[FaceCalculatedIndex[FaceIndex]];
                    int AdjacentBlockIndex = CommonMathematics.PositionToArray(AdjacentBlock, Gridsize);
                    if (!GameUtils.BlockFaceRenderCheck(AdjacentBlock, Gridsize, BuildableGrid, AdjacentBlockIndex, CalcuatedData, FaceCalculatedIndex[FaceIndex],i)) continue;

                    // Face looking at surface so we will render

                    for (int VertexIndex = 0; VertexIndex < BlockVertexFaceLenght; VertexIndex++)
                    {
                        if (CalcuatedData.BlockVerticies[FaceCalculatedIndex[FaceIndex], VertexIndex] == -Vector3.one) continue;
                        BlockVerticies[VertsCount + VertexIndex] = CalcuatedData.BlockVerticies[FaceCalculatedIndex[FaceIndex], VertexIndex] + new Vector3(CalculatedBlockPosition.x, CalculatedBlockPosition.y, CalculatedBlockPosition.z);
                    }

                    for (int TriangleIndex = 0; TriangleIndex < BlockTrianglesFaceLenght; TriangleIndex++)
                    {
                        if (CalcuatedData.BlockTriangles[FaceCalculatedIndex[FaceIndex], TriangleIndex] == -1) continue;
                        BlockTriangles[TrianglesCount + TriangleIndex] = CalcuatedData.BlockTriangles[FaceCalculatedIndex[FaceIndex], TriangleIndex] + VertexStartIndex;
                    }
