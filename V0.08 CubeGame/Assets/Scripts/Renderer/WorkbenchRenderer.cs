        ...
        public Mesh Calculate(GridBlock[] BuildableGrid, int3 Gridsize)
        {
            ...
            for (int i = 0; i < BuildableGrid.Length; i++)
            {
                ...
                for (int FaceIndex = 0; FaceIndex < CompiledBlockAtlas.BlockAtlas[BuildableGrid[i].ID].FaceCount; FaceIndex++)
                {
                    ...
                    {
                        if (CalcuatedData.BlockVerticies[CalculatedFaceIndex, VertexIndex] == -Vector3.one) continue;
                        BlockVerticies[VertsCount + VertexIndex] = CalcuatedData.BlockVerticies[CalculatedFaceIndex, VertexIndex] + new Vector3(CalculatedBlockPosition.x, CalculatedBlockPosition.y, CalculatedBlockPosition.z);
                    }

                    for (int TriangleIndex = 0; TriangleIndex < BlockTrianglesFaceLenght; TriangleIndex++)
                    {
                        if (CalcuatedData.BlockTriangles[CalculatedFaceIndex, TriangleIndex] == -1) continue;
                        BlockTriangles[TrianglesCount + TriangleIndex] = CalcuatedData.BlockTriangles[CalculatedFaceIndex, TriangleIndex] + VertexStartIndex;
                    }
