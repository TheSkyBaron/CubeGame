...
namespace Game.Player
{
    public class PlayerWorkbenchController : MonoBehaviour
    {
        ...
        {
            if (Is_PlayerHoldingScreen && !IsPlayerFrozen) PlayerCurrentRotation += CTX.ReadValue<Vector2>() * PlayerSettings.MouseSensevity;
        }
        ...
        private void PlayerPlaceBlock(InputAction.CallbackContext CTX)
        {
            ...
            if (Is_Removing && HitPointRemove != -Vector3.one)
            {
                ...
                Current.GridSystem.SetBlock(RemovePoint, 0, PlayerBlockRotationVector);
            }
        ...
        private void PlayerAlternativeMove(Vector2 RotationInput)
        {
            ...
            if (RotationInput.x != 0)
            {
                ...
                if (RotationInput.x == -1) RotationCalculation.y += 1;
            }
            if (RotationInput.y != 0)
            {
                if (RotationInput.y == -1) RotationCalculation.x += 3;
                if (RotationInput.y == 1) RotationCalculation.x += 1;
            }
        ...
        {
            GhostObject.GetComponent<MeshFilter>().sharedMesh = GameUtils.SingleBlockRenderer(NewBlock);
